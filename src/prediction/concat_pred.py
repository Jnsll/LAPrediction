...
def concat_prediction_HError_results_with_subcatchment_for_all_sites(approx=0, chronicle=0, permeability=27.32):
    ...
    df.to_csv(MYDIR + "Prediction_HErrorValues_SubCatch_Chronicle" + str(chronicle) + "_Approx"
                    + str(approx)
                    + "_K"
                    + str(permeability) + "_AllSites_Slope_Elevation_LC_SAR_Area_CV_HV_clean.csv", index=False)
    print(
        "File '"
        + "Prediction_HErrorValues_SubCatch_Chronicle" + str(chronicle) + "_Approx"
        + str(approx)
        + "_K"
        + str(permeability) + "_AllSites_Slope_Elevation_LC_SAR_Area_CV_HV_clean.csv"
        + "' has been created."
    )
...
def concat_prediction_pmax_results_for_all_sites():
    ...
    for approx in approximations:
        for chronicle in chronicles:
            for test_site in SITES_NUMBERS:
                file = (
                    MYDIR
                    + "Approx"
                    + str(approx)
                    ...
                    + "_K"
                    + str(permeability)
                    + "_BVE_CVHV_Geomorph_Sat_Extend_clean.csv"
                )
                try:
                    dfp = pd.read_csv(file, sep=";")
                except:
    ...
    df.to_csv(MYDIR + "Pred_Chronicle" + str(chronicle) + "_Approx"
                    + str(approx)
                    + "_K"
                    + str(permeability) + "_AllSites_BVE_CVHV_Geomorph_Sat_Extend_clean.csv", index=False)
    print(
        "File '"
        + "Pred_Chronicle" 
        + str(chronicle) 
        ...
        + str(approx)
        + "_K"
        + str(permeability) + "_AllSites_BVE_CVHV_Geomorph_Sat_Extend_clean.csv"
        + "' has been created."
    )
...
if __name__ == '__main__':
    ...
    concat_prediction_HError_results_with_subcatchment_for_all_sites()