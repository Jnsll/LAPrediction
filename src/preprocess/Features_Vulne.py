...
def compute_feature_CV_HV(site_number):
    ...
    with open("/run/media/jnsll/b0417344-c572-4bf5-ac10-c2021d205749/exps_modflops/results/" + str(site_name) +  "/" + 'Feature_CV_HV_Site_' + str(site_number) + '_BVE.csv', 'w') as f:
...
def get_mask_data_for_a_site(site_number):
    mask_file = os.path.join("/DATA/These/OSUR/Extract_BV_june/", str(site_number) + "_ZV_C1_H2_Mask.tif")
    print(mask_file)
