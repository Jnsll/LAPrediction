...
def all_random_forest(approx, chronicle, test_site):
    ...
    df_Chr_Approx, df_Chr_Approx_SiteTest = get_data_with_chronicle_number_and_site_number_as_test(df, approx, chronicle, test_site)
...
def get_scores_testSite(forest, X_testSite, y_testSite, y_train, y_train_pred):
    ...
    print('MSE test Site train : %.3f, test: %.3f' % (mean_squared_error(y_train, y_train_pred), mean_squared_error(y_testSite, y_testSite_test)))
    print('R^2 test Site train: %.3f, test: %.3f' % (r2_score(y_train, y_train_pred),r2_score(y_testSite, y_testSite_test)))
